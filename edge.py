...
class DQN(object) :
    ...
    def set_weight(self, receive):
        ...
        for i in range(16):
            if weight2_list == []:
                ...
                weight2_list[0] = np.append(weight2_list[0], np.array([receive[400 + (i*16):400 + (i+1)*16]]), axis=0)

        for i in range(16):
            if weight3_list == []:
                weight3_list.append(np.array([receive[656:661]]))
            else:
                weight3_list[0] = np.append(weight3_list[0], np.array([receive[656 + (i*5):656 + (i+1)*5]]), axis=0)
    ...
    def send_mess(self, reward):
        ...
        observation = struct.pack('1i401d256d80d', 0, reward, *weight1, *weight2, *weight3)
    ...
    def plot(self):
        ...
        ax.plot(x, r)
        ax.set_title('highway-v0-Episode_reward')
